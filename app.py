...
def apply_filters(df, dates, issue_type, analyst):
    ...
    Aplica filtros globais da barra lateral.
    """
    temp_df = df.copy()
...
def file_preview_popover(file_path_str, filters, label="Preview Data"):
    """
    Interactive data preview and export.
    Prévia interativa e exportação de dados.
    """
...
def load_analytical_data():
    """
    Loads Gold analytical layer.
    Carrega a camada analítica Gold.
    """
...
def load_json_meta(path_key):
    """
    Safely loads JSON metadata.
    Carrega metadados JSON com segurança.
    """
...
with st.sidebar:
    ...
    st.divider()
    if stats: 
        st.sidebar.caption(f"Sync: {stats.get('execution_date', 'N/A')}")
...
if page == "📈 Executive Dashboard":
    ...
    if df_master is not None:
        ...
        # Tabs with multi-level drill down
        # Abas com detalhamento multinível
        ...
        # Issue type drill-down
        # Detalhamento por tipo de chamado
        with tab_type:
...
# Página 2: Logs de Auditoria do Pipeline
            ...
            if "bronze" in wf:
                with st.container(border=True):
                    ...
                    for task in wf["bronze"].get('tasks', []):
                        ...
                        if "file" in task: 
                            file_preview_popover(task['file'], current_filters, "Inspect Bronze")
            if "gold" in wf:
                with st.container(border=True):
                        ...
                        if "outputs" in task:
                            for out in task['outputs']: 
                                if out.endswith('.parquet'): 
                                    file_preview_popover(out, current_filters, f"Inspect Gold")
        with col_r:
            if "silver" in wf:
                with st.container(border=True):
                    ...
                    for task in wf["silver"].get('tasks', []):
                        st.write(f"✅ **{task['name']}**")
                        if "file" in task: 
                            file_preview_popover(task['file'], current_filters, "Inspect Silver")
            if "quality" in wf:
                with st.container(border=True):
                    ...
                    with st.popover("🔍 Inspect All Process Steps & Export"):
                        ...
                        if "steps" in stats:
                            ...
                            for q in q_tasks:
                                ...
                                if logic_info: 
                                    st.caption(f"💡 Logic: {logic_info.get('logic_pt')}")
                            st.divider()
